...
import uk.gov.hmrc.individualincomedesstub.domain.ErrorInvalidPathParameter
import uk.gov.hmrc.individualincomedesstub.domain.JsonFormatters.errorInvalidRequestParameterFormat

package object Binders {

  implicit def ninoBinder = new PathBindable[Nino] {

    private def errorResponse = {
      Json.toJson(ErrorInvalidPathParameter("Malformed nino submitted")).toString
    }

    override def bind(key: String, value: String): Either[String, Nino] = try {
      Right(Nino(value))
    } catch {
      case _: Throwable => Left(errorResponse)
    }

    override def unbind(key: String, value: Nino): String = value.nino
  }

  implicit def employerReferenceBinder = new PathBindable[EmployerReference] {

    private def errorResponse = {
      Json.toJson(ErrorInvalidPathParameter("Invalid employer reference submitted")).toString
    }
