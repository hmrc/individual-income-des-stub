...
object JsonFormatters {
  ...
  implicit val testAddressFormat: OFormat[TestAddress] =
    Json.format[TestAddress]
  implicit val testOrganisationDetailsFormat: OFormat[TestOrganisationDetails] =
    Json.format[TestOrganisationDetails]
  implicit val testOrganisationFormat: OFormat[TestOrganisation] =
    Json.format[TestOrganisation]
  implicit val testIndividualFormat: OFormat[TestIndividual] =
    Json.format[TestIndividual]
  ...
  implicit val taxYearFormat: Format[TaxYear] = new Format[TaxYear] {
    override def reads(json: JsValue): JsResult[TaxYear] =
      JsSuccess(TaxYear(json.asInstanceOf[JsString].value))

    override def writes(taxYear: TaxYear): JsValue = JsString(taxYear.ty)
  }

  implicit val selfAssessmentTaxReturnFormat: OFormat[SelfAssessmentTaxReturn] =
  ...
  implicit val errorInvalidRequestFormat: Format[ErrorInvalidRequest] =
    new Format[ErrorInvalidRequest] {
