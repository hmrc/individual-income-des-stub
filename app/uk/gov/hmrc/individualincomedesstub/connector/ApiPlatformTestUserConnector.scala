...
class ApiPlatformTestUserConnector @Inject()(
  ...
  def getOrganisationByEmpRef(empRef: EmpRef)(
      implicit hc: HeaderCarrier): Future[Option[TestOrganisation]] = {
    http.GET[TestOrganisation](
      s"$serviceUrl/organisations/empref/${empRef.encodedValue}") map (Some(_))
  } recover {
    case e: NotFoundException =>
      logger.warn(
        s"unable to retrieve employer with empRef: ${empRef.value}. ${e.getMessage}")
      None
  }

  def getIndividualByNino(nino: Nino)(
      implicit hc: HeaderCarrier): Future[TestIndividual] = {
    http.GET[TestIndividual](s"$serviceUrl/individuals/nino/${nino.value}")
  } recover {
    case e: NotFoundException =>
      logger.warn(
        s"unable to retrieve individual with nino: ${nino.value}. ${e.getMessage}")
